...
::: tip
此文件為 [Android 開發規範(完结版)](https://github.com/Blankj/AndroidStandardDevelop) 之翻譯，如有侵犯著作權等行為煩請另行告知，也推薦此作者的其他相關 Demo：[Android 開發人員不得不收集的代碼](https://github.com/Blankj/AndroidUtilCode)。
:::
...
2. 編碼格式統一為 **UTF-8** 。
3. 編輯完 .java、.xml 等檔案後一定要 **格式化** (如果團隊有制定好的規則，就優先遵照團隊規則，否則統一使用 Android Studio 默認範本即可)。
4. 刪除多餘的 import，減少警告出現，可利用 Android Studio 的 Optimize Imports(Settings -> Keymap -> Optimize Imports) 快捷鍵。
...
::: warning
即使純拼音命名方式也要避免使用。 但alibaba、taobao、youku、hangzhou等國際通用的名稱，可視同英文。
:::
...
* package 有私有作用網域(package-private scope)
  ...
  按照分層原則，我們還得放在 util 包下，好像不太合適，但放在其它包更不合適，功能越來越多，util類也越定義越多。 
...
* 高度抽象
  ...
  PBF 從確定 AppName 開始，根據功能模組劃分 package，再考慮各個部分的具體實現細節，而 PBL 從一開始就要考慮要不要 dao 層，要不要 com 層等等。
...
* 只通過 class 來分離邏輯代碼
  ...
  實際上通常為了方便，直接 import service 包，Service 後綴是為了避免引入的 class 和當前包下的 class 命名衝突。
...
當然，谷歌也有相應的 Sample：**[todo-mvp](https://github.com/android/architecture-samples/tree/todo-mvp)**，其結構如下所示：
```
com
└── example
    └── android
...
└── domain
    └── app
        ├── App.java 定義Application類
        ├── Config.java 定義配置資料(常量)
        ├── base 基礎元件
        ├── custom_view 自定義視圖
        ├── data 資料處理
        │   │── DataManager.java 資料管理員，
...
### 2. 類別名稱
類別名稱以`UpperCamelCase`風格編寫，通常是名詞或名詞短語，介面名稱有時可能是形容詞或形容詞短語。 
現在還沒有特定的規則來命名注解類型。
...
| `Service`類 | 以`Service`為後綴標識 | 時間服務`TimeService` |
...
| 方法 | 說明|
| :- | :- |
| `initXX()` | 初始化相關方法，以init為前置標識 |
| `isXX()`, `checkXX()` | 方法返回值為boolean型的請以is/check為前綴標識 |
| `getXX()` | 返回某個值的方法 |
| `setXX()` | 設置某個屬性值 |
| `handleXX()`, `processXX()` | 對數據進行處理的方法 |
| `displayXX()`, `showXX()` | 彈出提示框和提示資訊，使用display/show為前綴標識 |
| `updateXX()` | 更新數據 |
| `saveXX()`, `insertXX()` | 保存或插入數據 |
| `resetXX()` | 重置數據 |
| `clearXX()` | 清除數據 |
| `removeXX()`, `deleteXX()` | 拿掉資料或者檢視等 |
| `drawXX()` | 繪製資料或效果相關的，以draw為前置識別 |
...
::: warning
所有的 VO(值物件) 統一採用標準的`lowerCamelCase`風格編寫，所有的 DTO(數據傳輸物件) 就按照介面文檔中定義的欄位名稱編寫。
:::

#### 5-1 scope(範圍)
非公有，非靜態欄位命名以 m 開頭。 靜態欄位命名以 s 開頭。 其他欄位以小寫字母開頭。 
例如：
```java
...
#### 5-2 Type0(控件類型)
考慮到 Android 眾多的 UI 控件，為避免控件和普通成員變數混淆及更好地表達意思，所有用來表示控制項的成員變數統一加上控制項縮寫作為前綴標識(具體見附錄[UI 控件縮寫表](#ui-控制件縮寫表))。
...
### 2. 編寫簡短方法
在可行的情況下，盡量編寫短小精練的方法。
如果某個方法的代碼超出40行，請考慮是否可以在不破壞程式結構的前提下對其拆解。
...
如果類繼承於 Android 元件(例如`Activity`或`Fragment`)，那麼把重寫函數按照他們的生命週期進行排序是一個非常好的習慣。
例如，`Activity`實現了`onCreate()`、`onDestroy()`、`onPause()`、`onResume()`，它的正確排序如下所示：
```java
public class MainActivity extends Activity {
...
Android Studio 已幫你寫好了相關的 Live Templates，只需要在和啟動相關`Activity`的內部輸入`starter`，即可生成它的啟動器，如下所示：
```java
...
同理，在和啟動相關`Fragment`的內部輸入`newInstance`即可，如下所示：
```java
...
* 函數鏈的換行
  ...
  當同一行中調用多個函數時(例如使用構建器時)，對每個函數的調用應該在新的一行，我們把換行符插入在`.`之前。 
  例如：
  ```java
...
如果是三方庫開發，其使用到的資源檔及相關的`name`都應該使用庫名作為前綴標識，這樣做可以避免三方庫資源和實際應用資源重名的衝突。

### 1. 動畫資源檔案 (anim/ 和 animator/)
安卓主要包含屬性動畫和視圖動畫，其視圖動畫包括補間動畫和逐幀動畫。 
...
### 2. 顏色資源檔案(color/)
專門存放顏色相關的資源檔。 
也可以放於`res/drawable/`目錄，引用時則用`@drawable`來引用，但不推薦這麼做。
...
::: warning
使用 Android Studio 的外掛程式 SelectorChapek 可以快速生成 selector，前提是命名要規範。
:::
...
例如可以有多個`style`檔，如`styles.xml`、`styles_home.xml`、`styles_item_details.xml`、`styles_forms.xml`。

#### 6-1 colors.xml
`<color>`的`name`命名使用下劃線命名法，在你的`colors.xml`檔案中應該只是映射顏色的名稱一個 ARGB 值，而沒有其它的。 不要使用它為不同的按鈕來定義 ARGB 值。
```xml
  <resources>
...
同時，這些定義是跟一些環境關聯起來的，如`button`或者`comment`，應該放到一個按鈕風格中，而不是在`colors.xml`檔中。
...
如果在專案中有用BufferKnife的話，使用 Android Studio 的外掛程式：ButterKnife Zelezny，方便讓你說明所生成之註解；沒用BufferKnife的話可以使用 Android Code Generator 外掛程式。
...
### 2. 方法註釋
每一個成員方法(包括自定義成員方法、覆蓋方法、屬性方法)的方法頭都必須做方法頭註釋，在方法前一行輸入`/** + Enter`或者設置`Fix doc comment`(Settings -> Keymap -> Fix doc comment)快捷鍵，AS便會幫你生成範本，我們只需要補全參數即可，例如：
```java
/**
 * bitmap轉byteArr
 *
...
### 4. 其他一些註釋
AS已幫你集成了一些註釋範本，我們只需要直接使用即可，在代碼中輸入`todo`、`fixme`等這些註釋範本，回車後便會出現如下註釋：
```java
...
有時一個類可能包含大量的方法，同時需要對每個方法進行幾次測試。 在這種情況下，建議將測試類分成多個類。 
...
6. 資料提供統一的入口。 無論是在 MVP、MVC 還是 MVVM 中，提供一個統一的數據入口，都可以讓代碼變得更加易於維護。 
   例如可使用一個`DataManager`，把`HTTP`、`preference`、`eventpost`、`database`都放在`DataManager`裡面進行操作，我們只需要與`DataManager`打交道。
7. 多用組合，少用繼承。
8. 提取方法，去除重複代碼。 
...
19. 盡量在合適的場合使用單例。 使用單例可以減輕載入的負擔、縮短載入的時間、提高載入的效率，但並不是所有地方都適用於單例，簡單來說，單例主要適用於以下三個方面：
    * 控制資源的使用，通過線程同步來控制資源的併發訪問。
    * 控制實例的產生，以達到節約資源的目的。
    * 控制數據的共用，在不建立直接關聯的條件下，讓多個不相關的進程或線程之間實現通信。
    
20. 把一個基本數據類型轉為字串，`基本數據類型.toString()`是最快的方式，`String.valueOf(數據)`次之，`數據 + ""`最慢。
21. 使用 Android Studio 自帶的 Lint 來最佳化代碼結構(右鍵 module、目錄或者檔案，選擇 Analyze -> Inspect Code)。
22. 最後不要忘了記憶體洩漏的檢測。
